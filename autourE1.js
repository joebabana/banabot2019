...
var conf = require('./config_autourE1.js');
var dsteem = require('dsteem');
var anAPI = new ApiService(conf.apihost);
var client = new dsteem.Client(conf.steemnode);
var sleephours = typeof conf.sleephours == 'number'? conf.sleephours*60*60*1000 : 1*60*60*1000;
...
async function getUnexploredCoordinates(x, y, size) {
    ...
    let rawCoordinates = new map2Dee(x, y, size, 0);
...
joblog.length = 0; //emptying array.
    for (var aProfile of conf.profile.credentials) {
		...
		await timeout(3000);
            let explorerships = fleetList.find(({ type }) => type == 'explorership');
            ...
            clog(`${aPlanet.name} - Number of explorersI on this planet : ${explorerships.quantity}`);
                ...
                if (explorerships.quantity > 0) {
                    ...
                    if (noOfLaunch > 0) {
                        ...
                        if (potentialTargets.length > 0) {
                            let randomizeTargets = (conf.randomize)? fisherYates(potentialTargets): potentialTargets; // Fisher Yates Randomizer.
                            for (var i = 0; i < noOfLaunch; i++) {
                                ...
                                await sendCustomJSON(aTarget.x, aTarget.y, aPlanet.id, key, _username, 'explorership');
                                } else {
    ...
    setTimeout(runAll, sleephours); // 1 hour interval.
}
